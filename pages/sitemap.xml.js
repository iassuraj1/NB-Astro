...
const staticRoutes = [
  ...
  { loc: '/astrologer-courses', changefreq: 'weekly', priority: '0.8' },
  { loc: '/contact', changefreq: 'monthly', priority: '0.7' },
  { loc: '/blog', changefreq: 'daily', priority: '0.8' }, // ✅ Blog list
];
...
export const getServerSideProps = async ({ res }) => {
  ...
  // ✅ Fetch blogs
  const blogResult = await fetchJson('/api/blog?limit=100', {});
  ...
  const blogRoutes = blogs
    .filter((blog) => blog?.slug && blog?.isPublished !== false)
    .map((blog) => ({
      loc: `/blog/${blog.slug}`,
      lastmod: blog.updatedAt || blog.publishedAt || blog.createdAt || today,
      changefreq: 'weekly',
      priority: '0.8',
    }));

  // ✅ All routes combined
  const allRoutes = [...staticRoutes, ...courseRoutes, ...consultationRoutes, ...blogRoutes];

  const xml = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
${allRoutes.map(toUrlXml).join('')}
</urlset>`;
